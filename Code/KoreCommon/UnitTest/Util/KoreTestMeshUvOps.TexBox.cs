...
//         double uvMargin = 0.1;
//         double uvSep = 0.1;
//         double uvBoxWidth = (1 - (uvMargin * 2) - (uvSep * 2)) / 3; // 1 = total, minus a margin or each side, plus 2 separators across the square
//         double uv0 = uvMargin;
//         double uv1 = uv0 + uvBoxWidth;
//         double uv2 = uv1 + uvSep;
//         double uv3 = uv2 + uvBoxWidth;
//         double uv4 = uv3 + uvSep;
//         double uv5 = uv4 + uvBoxWidth;

//         KoreUVBox topUvBox = new (new KoreXYVector(uv2, uv0), new KoreXYVector(uv3, uv1));
//         KoreUVBox frontUvBox = new (new KoreXYVector(uv2, uv2), new KoreXYVector(uv3, uv3));
//         KoreUVBox bottomUvBox = new (new KoreXYVector(uv2, uv4), new KoreXYVector(uv3, uv5));
//         KoreUVBox leftUvBox =  new (new KoreXYVector(uv0, uv2), new KoreXYVector(uv1, uv3));
//         KoreUVBox rightUvBox = new (new KoreXYVector(uv4, uv2), new KoreXYVector(uv5, uv3));
//         KoreUVBox backUvBox = new (new KoreXYVector(uv0, uv0), new KoreXYVector(uv1, uv1));

//         // Define the vertices of the cube
//         KoreXYZVector v0 = new (-size, -size, -size); // bottom right
//         KoreXYZVector v1 = new (size, -size, -size); // bottom left
//         KoreXYZVector v2 = new (size, size, -size); // top left
//         KoreXYZVector v3 = new (-size, size, -size); // top right

//         KoreXYZVector v4 = new (-size, -size, size); // bottom right
//         KoreXYZVector v5 = new (size, -size, size); // bottom left
//         KoreXYZVector v6 = new (size, size, size); // top left
//         KoreXYZVector v7 = new (-size, size, size); // top right
