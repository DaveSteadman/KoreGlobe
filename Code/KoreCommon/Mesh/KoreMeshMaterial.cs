...
public record struct KoreMeshMaterial(
    string Name,                // Material name for GLTF export/import and identification
    KoreColorRGB BaseColor,     // Includes RGBA - alpha channel handles transparency
    float Metallic = 0.0f,      // 0 = dielectric (plastic/wood), 1 = metallic
    float Roughness = 0.7f      // 0 = mirror smooth, 1 = completely rough
)
{
    // --------------------------------------------------------------------------------------------
    ...
    public static KoreMeshMaterial White => KoreMeshMaterialPalette.GetMaterial("White");
